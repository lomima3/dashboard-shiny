...
observeEvent(input$validerplz, {
  if (!is.null(input$validerplz) && input$validerplz == 1) {
    dsstr <- fn_GetDataStructure(uploadData$fiData)
    imputeOptionsList <- lapply(dsstr$Variable, function(x) {
      dmClass <- input[[paste0("rb", x)]]
      if (dmClass %in% c("Variable qualitative", "Variable quantitative")) {
        choices <- switch(dmClass,
                          "Variable qualitative" = c("Mode", "Ne pas Imputer"),
                          "Variable quantitative" = c("Moyenne", "Médiane", "Ne pas Imputer"))
        radioButtons(
          paste0("impute", x),
          x,
          choices = choices,
...
observeEvent(input$applyChanges, {
  if (!is.null(uploadData$fiData)) {
    ...
    for (var in cols_with_missing) {
      ...
      } else if (imputation_method == "Médiane") {
